...
class MainTests: XCTestCase {
  ...
  let testRepo = "Kitura-Starter-Bluemix"
  ...
  func testTrackerJsonBuilding() {
    ...
    let tracker = MetricsTrackerClient(configMgr: configMgr, repository: testRepo)
  ...
  func testNumerousServiceJson() {
    ...
    let configMgr = ConfigurationManager()
    configMgr.load(jsonOptions)
    let tracker = MetricsTrackerClient(configMgr: configMgr, repository: testRepo, codeVersion: testCodeVersion)
    guard let jsonResult = tracker.buildTrackerJson(configMgr: configMgr) else {
      XCTFail("Failed to receive json from build tracker method.")
      return
    }
